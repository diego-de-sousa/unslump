...
import { workoutSession, currentWorkoutData } from '../../stores/workoutController';
          ...
          <div class="p-4">
            ...
            <div class="space-y-3">
              {phases.map(([phaseId, phase], phaseIndex) => {
                ...
                return (
                  <div key={phaseId} class="rounded-lg overflow-hidden shadow-sm" style={{ border: `2px solid ${phase.colorPrimaryHex || '#4f46e5'}` }}>
                    {/* Phase Header */}
                    <div
                      class="px-4 py-3 font-bold text-base"
                      style={{
                        backgroundColor: phase.colorPrimaryHex || '#4f46e5',
                        color: 'white'
                      }}
                    >
                        ...
                        return (
                          <button
                            ...
                            class={`
                              ...
                              ${isCurrent ? 'bg-blue-50 font-semibold border-l-4' : 'border-l-4 border-transparent'}
                              ${status === 'completed' ? 'bg-green-50 text-gray-700 border-l-green-500' : ''}
                              ${status === 'skipped' ? 'bg-orange-50 text-gray-500 border-l-orange-400' : ''}
                              ${status === 'pending' && !isCurrent ? 'text-gray-700 hover:bg-gray-50' : ''}
                            `}
                            style={isCurrent ? { borderLeftColor: phase.colorPrimaryHex || '#4f46e5' } : {}}
                          >
                            <div class="flex items-center justify-between">
                              ...
                              <span class="ml-2 flex items-center gap-1">
                                ...
                                {exercise.duration > 0 && (
                                  <span class="text-xs text-gray-400">
                                    {(exercise as any).sides && (exercise as any).sides > 1
                                      ? `${exercise.duration}s x ${lang === 'es' ? 'lado' : 'side'}`
                                      : (exercise as any).sets && (exercise as any).sets > 1
                                        ? `${(exercise as any).sets}x${exercise.duration}s`
                                        : `${exercise.duration}s`
                                    }
                                  </span>
                                )}
